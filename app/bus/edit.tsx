...
export default function EditBus() {
  ...
  // Step 1 State - pre-filled with mockup data
  const [regNumber, setRegNumber] = useState('MH-02-AB-1234');
  ...
  useEffect(() => {
    if (mounted && step === 1 && seats.length === 0) {
      // Mock initializing grid for edit
      const numRows = 6;
      const numCols = 4;
      const newSeats: Seat[] = [];
    ...
    return (
      <View className="flex-1 items-center justify-center bg-white px-10">
        <View className="w-full items-center rounded-3xl border border-slate-100 bg-white p-8 shadow-sm">
          ...
          <Text className="mb-1 text-center text-lg font-bold text-[#64748B]">Bus Updated</Text>
    ...
    <SafeAreaView className="flex-1 bg-white" edges={['top']}>
      ...
      <View className="flex-row items-center px-6 py-4">
        ...
        <View className="mr-8 flex-1 items-center">
          <Text className="text-[20px] font-bold text-[#848F4B]">Edit</Text>
        </View>
      </View>
      ...
      <ScrollView
        ...
        {step === 2 && (
          <View className="px-6">
            <Text className="mb-1 text-xl font-bold text-[#64748B]">Seats</Text>
